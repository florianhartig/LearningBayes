...
As we may need to use R libraries that require a C++ compiler, Windows users should install Rtools (the C++ compiler for R). Either download and install Rtools directly from [here](https://cran.r-project.org/bin/windows/Rtools/) or run the following commands in RStudio
...
If you want to run code from the book, you can copy and paste it to your RStudio, but for your convenience, I also provide a plain R version of the code of each chapter [here](https://github.com/florianhartig/LearningBayes/tree/master/classroom/).
