...
::: callout-note
In this chapter, we will discuss approximate Bayesian inference, for situations where the likelihood of a (stochastic simulation) model cannot be calculated directly. You will learn

- Why exact likelihood calculation can be infeasible for stochastic simulation models
- The two main simulation-based inference strategies: Approximate Bayesian Computation (ABC) and synthetic likelihood
- How to implement ABC-rejection and ABC-MCMC algorithms based on summary statistics
- A worked example: fitting a stochastic movement model with ABC
:::
...
Let's create some data with known parameters
...
syntheticLogLikelihood <- function(par){
  # simulate several model runs at the given parameter values
  simulatedData <- matrix(NA, nrow = 100, ncol = 2)
  for (j in 1:100){
    simulation <- stochasticModel(par)
    simulatedData[j,] <- c(mean(simulation) , sd(simulation))
  }
  # fit a normal distribution to each summary statistic across the 100 replicates
  syntheticLikelihood1 <- fitdistr(simulatedData[,1], "normal")
  syntheticLikelihood2 <- fitdistr(simulatedData[,2], "normal")

  prob1 <- dnorm(meandata-syntheticLikelihood1$estimate[1], 
                 sd = syntheticLikelihood1$estimate[2], log = T)
  prob2 <- dnorm(standarddeviationdata-syntheticLikelihood2$estimate[1], 
                 sd = syntheticLikelihood2$estimate[2], log = T)
...
run_MCMC_Synthetic <- function(startvalue, iterations){
    ...
    chain = array(dim = c(iterations+1,2))
    chain[1,] = startvalue
    currentLL <- syntheticLogLikelihood(chain[1,])
    ...
    for (i in 1:iterations){
        ...
        proposalLL <- syntheticLogLikelihood(proposal)

        if(runif(1) < exp(proposalLL - currentLL)){
            chain[i+1,] = proposal
...
posterior <- run_MCMC_Synthetic(c(3,-2),20000)
plot(posterior)
