...
* The indicators calculated by Bayesian inference carry a different information than what is contained in p-values and the MLE, and many researchers have argued that this information is more intuitive and helpful than what is provided by the frequestist indicators. 
...
We now want to evaluate the results of this experiment using the three main inferential methods in statistics: MLE, NHST and Bayes. 

### The statistical model

All three methods require that we have a statistical model, which allows us to express the probability of our possible observations as a function of the model's parameters. 

In the case of a coin flip, such a model is the binomial model, which has one pararameter, p, the probability of a sucess. In R, all simple statistical models have a name, which is complemented by a letter for calculating different aspects of the model. In this case, the name is binom, and then 

* rbinom (r = random) samples from a binomial distribution
* qbinom (q = quantile) gives the quantile function of the distribution
* pbinom (p = probability) gives the cummulative probability function
* dbinom (d = density) gives the probability (density) of a particular observation 

So, for example, dbinom(6,10,0.9) gives you the probability of obtaining 6/10 successes when the true probability of heads is 0.9
...
### The Bayesian posterior estimate
